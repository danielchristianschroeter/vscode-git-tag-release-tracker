...
suite("MultiRepoService Test Suite", () => {
  ...
  test("getAggregatedData should return aggregated data from multiple repositories", async () => {
    ...
    (services1.gitService.getCommitCounts as sinon.SinonStub)
      .withArgs("main", "feature/new-stuff", sinon.match.any)
      .resolves(3); // 3 unmerged
  ...
  test("invalidateCacheForRepo should set build statuses to loading", () => {
    ...
    assert.ok(
      (globals.statusBarService?.triggerUpdate as sinon.SinonStub).calledOnce,
      "triggerUpdate should be called"
    );
  });
  ...
  test("pollInProgressBuilds should refresh status for in-progress builds", async () => {
    ...
    assert.strictEqual(
      updatedData?.branchBuildStatus?.status,
      "success",
      "Branch build status should be updated to success"
    );
    assert.ok(
      (globals.statusBarService?.triggerUpdate as sinon.SinonStub).calledWith(false),
      "UI should be triggered to update"
    );

    await clock.tickAsync(15000);
  });

  test("pollInProgressBuilds should back off repeated polling for long-running builds", async () => {
    const clock = sandbox.useFakeTimers();
    ...
    (services.ciService.getBuildStatus as sinon.SinonStub)
      .withArgs("main", "github", false, true)
      .resolves({status: "in_progress"});

    await multiRepoService["pollInProgressBuilds"]();
    assert.strictEqual((services.ciService.getBuildStatus as sinon.SinonStub).callCount, 1);

    await multiRepoService["pollInProgressBuilds"]();
    assert.strictEqual(
      (services.ciService.getBuildStatus as sinon.SinonStub).callCount,
      1,
      "Should not poll again before the first backoff window expires"
    );

    await clock.tickAsync(15000);
    await multiRepoService["pollInProgressBuilds"]();
    assert.strictEqual((services.ciService.getBuildStatus as sinon.SinonStub).callCount, 2);
    ...
    assert.strictEqual(
      (services.ciService.getBuildStatus as sinon.SinonStub).callCount,
      2,
      "Second backoff window should stretch to 30 seconds"
    );

    await clock.tickAsync(15000);
    await multiRepoService["pollInProgressBuilds"]();
    assert.strictEqual((services.ciService.getBuildStatus as sinon.SinonStub).callCount, 3);
  });
  ...
  test("clearCache should clear all repository data", async () => {
    ...
    assert.strictEqual(multiRepoService["pollState"].size, 0);
  });
});
